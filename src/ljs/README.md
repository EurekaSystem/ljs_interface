...
```
ros2 action send_goal /skill/ljs ljs_skill_msgs/action/Ljs "skill_data: 'test data'" 
```
...
To implement your skill logic, have a look at `ljs/skill_impl.py`: this is
the main file you will have to modify.

You might want however to create a custom message type to start/configure/stop
your skill. Check the `ljs_skill_msgs` package that has been generated alongside
...
- run `ros2 run pal_deploy deploy --package ljs generic-XXc` (replace `generic-XXc` by your actual robot)

You can now `ssh` onto the robot (`ssh pal@generic-XXc`, password
`pal`), and go to your project: `cd ~/deployed_ws/share/ljs`
